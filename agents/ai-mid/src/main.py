...
Controls player 3 only. One AgentCore runtime per position, so each can run
its own model and fail independently of the other four.

    POST /invocations
    body: {"gameState": ..., "teamId": N, "myPlayers": [3]}
...
MY_PLAYER_INDEX = 3
ROLE = "MIDFIELDER"
ROLE_PROMPT = """You link the lines and you touch the ball most, so your choices compound. When a
teammate has it, sit just BEHIND the ball rather than ahead of it - holding is
worth more here than pushing on. Screen the middle when they have it. You are
usually the one the defenders are looking for, so stay findable."""
# -------------------------------------------------------------------------
